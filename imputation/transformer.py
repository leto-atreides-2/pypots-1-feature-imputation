...
class _TransformerEncoder(nn.Module):
    def __init__(
        ...
        if last_feature_only:
            print('Losses computed for the last feature only (modified Transformer).')
        else:
            print('Losses computed for all features (original Transformer).')
    ...
    def forward(self, inputs):
        ...
        if self.last_feature_only:
            ...
            learned_presentation = torch.unsqueeze(learned_presentation[:, :, -1], 2)
        ...
        imputation_loss = cal_mae(learned_presentation, X_intact, indicating_mask)
