...
class _SAITS(nn.Module):
    def __init__(
        ...
        if last_feature_only:
            print('Losses computed for the last feature only (modified SAITS).')
        else:
            print('Losses computed for all features (original SAITS).')
    ...
    def forward(self, inputs):
        ...
        if self.last_feature_only:
            X = torch.unsqueeze(X[:, :, -1], 2)
            masks = torch.unsqueeze(masks[:, :, -1], 2)
            X_tilde_1 = torch.unsqueeze(X_tilde_1[:, :, -1], 2)
            X_tilde_2 = torch.unsqueeze(X_tilde_2[:, :, -1], 2)
            X_tilde_3 = torch.unsqueeze(X_tilde_3[:, :, -1], 2)
        ...
        X_intact, indicating_mask = inputs["X_intact"], inputs["indicating_mask"]
        if self.last_feature_only:
            X_intact = torch.unsqueeze(X_intact[:, :, -1], 2)
            indicating_mask = torch.unsqueeze(indicating_mask[:, :, -1], 2)
        imputation_loss = cal_mae(X_tilde_3, X_intact, indicating_mask)
