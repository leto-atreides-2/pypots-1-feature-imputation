...
class RITS(nn.Module):
    ...
    def __init__(self, n_steps, n_features, rnn_hidden_size, device=None,
                 last_feature_only=False):
        super().__init__()
        self.n_steps = n_steps
        self.n_features = n_features
        self.rnn_hidden_size = rnn_hidden_size
        self.device = device
        self.last_feature_only = last_feature_only
    ...
    def impute(self, inputs, direction):
        ...
        for t in range(self.n_steps):
            ...
            reconstruction_loss += cal_mae(torch.unsqueeze(x_h[:, -1], 1), torch.unsqueeze(x[:, -1], 1),
                                           torch.unsqueeze(m[:, -1], 1)) \
                if self.last_feature_only else cal_mae(x_h, x, m)

            x_c = m * x + (1 - m) * x_h

            z_h = self.feat_reg(x_c)
            reconstruction_loss += cal_mae(torch.unsqueeze(z_h[:, -1], 1), torch.unsqueeze(x[:, -1], 1),
                                           torch.unsqueeze(m[:, -1], 1)) \
                if self.last_feature_only else cal_mae(z_h, x, m)

            alpha = torch.sigmoid(self.combining_weight(torch.cat([gamma_x, m], dim=1)))

            c_h = alpha * z_h + (1 - alpha) * x_h
            reconstruction_loss += cal_mae(torch.unsqueeze(c_h[:, -1], 1), torch.unsqueeze(x[:, -1], 1),
                                           torch.unsqueeze(m[:, -1], 1)) \
                if self.last_feature_only else cal_mae(c_h, x, m)
...
class _BRITS(nn.Module):
    ...
    def __init__(self, n_steps, n_features, rnn_hidden_size, device=None,
        ...
        self.rnn_hidden_size = rnn_hidden_size
        # create models
        self.rits_f = RITS(n_steps, n_features, rnn_hidden_size, device, last_feature_only)
        self.rits_b = RITS(n_steps, n_features, rnn_hidden_size, device, last_feature_only)
        self.last_feature_only = last_feature_only
        if last_feature_only:
            print('Losses computed for the last feature only (modified BRITS).')
        else:
            print('Losses computed for all features (original BRITS).')
    ...
    def merge_ret(self, ret_f, ret_b):
        ...
        consistency_loss = self.get_consistency_loss(ret_f["imputed_data"][-1], ret_b["imputed_data"][-1]) \
            if self.last_feature_only else self.get_consistency_loss(ret_f["imputed_data"], ret_b["imputed_data"])
